...
/* Un fichier temporaire appelé "session" est créé sur le serveur avec un identifiant unique. Cette session est liée à un seul internaute car dans le même temps, un cookie est déposé sur le poste de l'internaute avec l'identifiant. Ce cookie se détruit lorsqu'on quiite le navigateur.
...
var_dump($_SESSION);    // nous voyons encore le contenu de la session ici. En effet, il faut savoir que session_destroy() est d'abord lu par l'interpréteur, puis exécuter réellement qu'à la toute fin du script : pour le voir, vérifier le dossier xampp/tmp/ où la session n'existe plus.
