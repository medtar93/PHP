...
$contenu_gauche .= '<div class="list-group" >';
    ...
    while ($cat = $resultat->fetch(PDO::FETCH_ASSOC)) {
        //debug($cat);    // on voit que les catégorie sont dans les array à l'indice "catégorie"
        $contenu_gauche .= '<a href="?categorie='. $cat['categorie'] .'" class="list-group-item">'. $cat['categorie'] .'</a>';  // on met l'array $cat['categorie] à la place de "tous" pour récupérer à chaque tour de boucle chacune des catégories présentent dans cet array (voir debug ci-dessous)
    }
...
while ($produit = $donnees->fetch(PDO::FETCH_ASSOC)) {
    // debug($produit);    // on a un array associatif avec un seul produit à l'intérieur à chaque tour de boucle
