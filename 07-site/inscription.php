...
if (!empty($_POST)) {  // si le formulaire est soumis
	...
	if (empty($contenu)) {   // si $contenu est vide c'est qu'il n'y a pas d'erreur
		...
		$membre = executeRequete("SELECT * FROM membre WHERE pseudo = :pseudo", array(':pseudo' => $_POST['pseudo']));   // on sélestionne en base les éventuels membres dont le pseudo correpond au pseudo donné par l'internaute lors de l'inscription.
		...
		if ($membre->rowCount() > 0) { 	//si la requête retourne 1 ou plusieurs résultats c'est que le pseudo existe en BDD
			...
			$inscription = true;	// pour ne plus ajouter le formulaire sur cette page
