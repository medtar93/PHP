...
function internauteEstConnecte() {
    if (isset($_SESSION['membre'])) {   // si la session "membre" existe, c'est que l'internaute est passé par la page connexion et que nous avons créé cet indice dans $_SESSION
        return true;
    } else {
        return false;
    }
...
function internauteEstConnecteEtAdmin(){
    ...
    if(internauteEstConnecte() && $_SESSION['membre']['statut'] == 1  ){ // Si membre connecté ET que sont statut dans la session vaut 1, il est admin connecté 
        return true; 
    }else{ 
        return false; 
    }    // OU : 
...
function executerequete ($req, $param = array()) {   // cette fonction attend 2 valeurs :  1 requête SQL (obligatoire) et un array qui associe les marqueurs aux valeurs (non obligatoire car on a affecté au paramètre $param un array() vide par défaut)
    ...
    if (!empty($param)) {  // si $param n'est pas vide, je peux faire la boucle :
        foreach ($_POST as $indice => $valeur) {
            $_POST[$indice] = htmlspecialchars ($valeur, ENT_QUOTES);   // on échappe les valeurs de $param que l'on remet à leur place dans $param[$indice]  
        }
    }
    ...
    $result = $pdo->prepare($req);  // on prépare la requête envoyé à notre fonction
