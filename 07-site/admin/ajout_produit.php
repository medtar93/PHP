...
if(!empty($_POST)) {
    ...
    // ici les code de la photo à venir
    $photo_bdd =''; // par défaut la photo est vide en BDD

    debug($_FILES);

    if (!empty($_FILES['photo']['name'])) { // si il y a un nom de fichier dans la superglobale $_FILES, c'est que je suis en train d'uploader un fichier
        $nom_photo = $_POST['reference'] . '_' . $_FILES['photo']['name'];  //pour créer un nom de fichier unique, on concatène la référence du produit avec le nomdu fichier en cours d'upload

        $photo_bdd = 'photo/' . $nom_photo;  //chemein relatif de la photo enregistré dans la BDD correspondant au fichier physique uploadé dans le dossier /photo du site
...
<form method="post" action="" enctype="multipart/form-data"><!-- multipart/form-data spécifie que le formulaire envoie des données d'un fichier uploadé et du texte : permet d'uploader une photo pour le produit-->
    ...
    <input type="radio" name="public" value="m">Féminin <br><br>
